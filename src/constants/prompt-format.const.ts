...
/** 用户输入回合：本回合结束后玩家将自由输入下一行动 */
export function buildUserInputTurnBlock(): string {
  return `【用户输入推进 — 台词为主】
玩家默认输入的是台词原文，须作为 MSG:你| 正文或极轻微润色；#(动作) 或 #（动作）为行为指令，非对白，须写进 NARR 或他人反应。
支持 @成员名 点名在场 NPC。每回合写完后，玩家继续输入；你须把局面推到「玩家想立刻输入」的钩子上。
禁止输出 CARD 或任何备选台词列表；回合尾部顺序：全部 MSG → MOOD → COMPLETE。`;
}
